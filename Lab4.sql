...
select s.nume, n.valoare, p.nume from studenti s join note n on n.nr_matricol = s.nr_matricol join didactic d on d.id_curs = n.id_curs join profesori p on p.id_prof = d.id_prof;
...
select s.nume from studenti s join note n on s.nr_matricol = n.nr_matricol join cursuri c on c.id_curs = n.id_curs where n.valoare = 10 and c.titlu_curs like 'BD';
...
select p.nume, p.prenume, c.titlu_curs from profesori p join didactic d on p.id_prof = d.id_prof right join cursuri c on c.id_curs = d.id_curs;
...
-- Construiti o listă cu studentii care s-au născut in aceeaşi zi grupându-i doi câte doi în ordine alfabetică a numelor (de exemplu in rezultat va apare combinatia Cobzaru-Pintescu dar nu va apare şi Pintescu-Cobzaru).
...
select s1.nume||'-'||s2.nume, to_char(s1.data_nastere, 'day') from studenti s1 join studenti s2 on s1.nume < s2.nume and to_char(s1.data_nastere, 'day') = to_char(s2.data_nastere, 'day') order by s1.data_nastere - next_day(s1.data_nastere, 'luni'), s1.nume, s2.nume;
...
select s1.nume||' '||s1.prenume||' a luat nota mai mare ca '||s2.nume||' '||s2.prenume||' la materia '||c1.titlu_curs from studenti s1 join note n1 on s1.nr_matricol = n1.nr_matricol join cursuri c1 on n1.id_curs = c1.id_curs, studenti s2 join note n2 on s2.nr_matricol = n2.nr_matricol join cursuri c2 on n2.id_curs = c2.id_curs where n1.valoare > n2.valoare and n1.id_curs = n2.id_curs;
...
select s1.nume, s2.nume, s1.data_nastere - s2.data_nastere from studenti s1 join studenti s2 on s1.nr_matricol != s2.nr_matricol order by s1.data_nastere - s2.data_nastere;
...
select s.nume, p.nume from studenti s join profesori p on length(s.nume) = length(trim(p.nume));

-- Afisati denumirile cursurilor la care s-au pus note cel mult egale cu 8 (<=8).
select distinct c.titlu_curs from cursuri c join note n on c.id_curs = n.id_curs minus select distinct c.titlu_curs from cursuri c join note n on c.id_curs = n.id_curs where n.valoare > 8;

-- Afisati numele studentilor care au toate notele mai mari ca 7 sau egale cu 7.
select s.nume, s.prenume from studenti s minus select s.nume, s.prenume from studenti s join note n on s.nr_matricol = n.nr_matricol where n.valoare < 7;

-- Sa se afiseze studentii care au luat nota 7 sau nota 10 la OOP intr-o zi de marti.
select s.nume, s.prenume, to_char(n.data_notare, 'day') from studenti s join note n on s.nr_matricol = n.nr_matricol join cursuri c on n.id_curs = c.id_curs where n.valoare in (7, 10) and c.titlu_curs like 'OOP' and to_char(n.data_notare, 'day') = 'marti';
...
select s.nume, s.prenume, n.valoare, trim(to_char(n.data_notare, 'MONTH'))||','||to_char(data_notare, 'yyyy') "Sesiunea", decode(round(data_notare, 'MONTH') - trunc(data_notare, 'MONTH') - 30, 0, '+', '') from studenti s join note n on s.nr_matricol = n.nr_matricol where n.valoare > 5;