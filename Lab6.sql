...
select count(*), an, grupa from studenti group by an, grupa order by an, grupa;

-- Afișați numărul de studenți din fiecare grupă a fiecărui an de studiu și specificați câți dintre aceștia sunt bursieri.
select count(*), an, grupa, count(bursa) from studenti group by an, grupa;
...
select count(*), to_char(data_notare, 'DAY') from note group by to_char(data_notare, 'DAY') order by to_char(data_notare, 'DAY');
...
select s.nume, s.prenume, avg(n.valoare) from studenti s join note n on s.nr_matricol = n.nr_matricol group by s.nr_matricol, nume, prenume order by avg(n.valoare) desc;

-- Modificați interogarea anterioară pentru a afișa și elevii fără nici o notă. Media acestora va fi null.
select s.nume, s.prenume, avg(n.valoare) from studenti s left join note n on s.nr_matricol = n.nr_matricol group by s.nr_matricol, nume, prenume order by avg(n.valoare) desc;

-- Modificați interogarea anterioară pentru a afișa pentru elevii fără nici o notă media 0.
select s.nume, s.prenume, nvl(avg(n.valoare), 0) from studenti s left join note n on s.nr_matricol = n.nr_matricol group by s.nr_matricol, nume, prenume order by avg(n.valoare) desc;

-- Modificati interogarea de mai sus pentru a afisa doar studentii cu media mai mare ca 8.
select s.nume, s.prenume, nvl(avg(n.valoare), 0) from studenti s left join note n on s.nr_matricol = n.nr_matricol group by s.nr_matricol, nume, prenume having avg(n.valoare) > 8 order by avg(n.valoare) desc;
...
select s.nume, s.prenume, max(n.valoare), min(n.valoare), avg(n.valoare) from studenti s join note n on s.nr_matricol = n.nr_matricol group by s.nr_matricol, s.nume, s.prenume having min(n.valoare) >=7;

-- Afișați numele și mediile studenților care au cel puțin un număr de 3 note puse în catalog.
select s.nume, s.prenume, avg(n.valoare) from studenti s join note n on s.nr_matricol = n.nr_matricol group by s.nr_matricol, s.nume, s.prenume having count(n.valoare) >= 3;

-- Afișați numele și mediile studenților care au cel puțin un număr de 3 note diferite puse în catalog.
select s.nume, s.prenume, avg(n.valoare) from studenti s join note n on s.nr_matricol = n.nr_matricol group by s.nr_matricol, s.nume, s.prenume having count(distinct n.valoare) >= 3;

-- Afișați numele și mediile studenților din grupa A2 anul 3.
select s.nume, s.prenume, avg(n.valoare) from studenti s join note n on s.nr_matricol = n.nr_matricol where s.grupa like 'A2' and s.an = 3 group by s.nr_matricol, s.nume, s.prenume;
