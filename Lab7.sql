...
select nume, prenume 
	...
	where an in (select an 
		...
		and grupa in (select grupa 
						from studenti 
						where nume like 'Arhire'
						) 
		and nr_matricol not in (select nr_matricol 
									from studenti 
									where nume like 'Arhire'
									);

-- Pentru fiecare grupă afișați numele studenților care au obținut cea mai mică notă la nivelul grupei.
select min(val), grupa, an 
	from (select avg(valoare) as val, s.grupa, s.an 
			from studenti s join note n on s.nr_matricol = n.nr_matricol 
			group by s.nr_matricol, s.grupa, s.an
			) 
	group by grupa, an;
...
select s.nume, s.prenume, avg(n.valoare) 
	...
	having avg(n.valoare) > (select avg(valoare) 
								from note
								);
...
-- Afișați numele studentului (studenților) cu cea mai mare medie precum și valoarea mediei
-- (atenție: limitarea numărului de linii poate elimina studenții de pe poziții egale; găsiți altă soluție).
select s.nume, s.prenume, avg(n.valoare) 
	from studenti s join note n on s.nr_matricol = n.nr_matricol 
	group by s.nr_matricol, s.nume, s.prenume 
	having avg(n.valoare) = (select max(val) 
								from (select avg(valoare) as val 
										from note 
...
select s.nume, s.prenume 
	...
	where c.titlu_curs like 'Logica' 
		and s.nume not like 'Ciobotariu' 
		and s.prenume not like 'Ciprian' 
		and n.valoare = (select n.valoare 
							from studenti s join note n on s.nr_matricol = n.nr_matricol join cursuri c on n.id_curs = c.id_curs 
							where s.nume like 'Ciobotariu' 
								and s.prenume like 'Ciprian' 
...
select * 
	from studenti 
	where nr_matricol = (select nr_matricol 
							from (select * 
									from (select s.nr_matricol, sum(punctaj) medie 
											from studenti s join (select n.nr_matricol, n.valoare * credite punctaj 
																	from note n join cursuri c on n.id_curs = c.id_curs) n on s.nr_matricol = n.nr_matricol 
																	group by s.nr_matricol 
																	order by sum(punctaj) desc
																	) 
											where rownum < 4 
											order by medie
									) 
...
select s.nume, s.prenume, n.valoare, c.titlu_curs 
	from studenti s join note n on s.nr_matricol = n.nr_matricol join (select id_curs, max(valoare) max_val 
																		from note 
																		group by id_curs
																		) nmax on n.id_curs = nmax.id_curs join cursuri c on n.id_curs = c.id_curs 
	where n.valoare = nmax.max_val;